/**
 * Neo Survivors Idle - WebGL2 Version
 *
 * This is the WebGL2 version of the game, using native WebGL2 rendering
 * instead of PixiJS. It provides better performance and smaller bundle size.
 *
 * To switch between renderers:
 * - For PixiJS version: use main.js
...
try {
  ...
  console.log(`[Neo Survivors] Using ${gameRenderer.isWebGL2() ? 'WebGL2' : 'Canvas 2D'} renderer`);
} catch (e) {
  console.error('[Neo Survivors] Failed to initialize renderer:', e);
  throw e;
}

// Convert palette to hex numbers
const paletteHex = palette.map((color) => parseInt(color.replace('#', ''), 16));
const colors = {
  player: 0x7dd3fc,
  collect: 0x6ee7b7,
  bulletLow: 0xfff7ed,
...
if (versionBadge) {
  versionBadge.textContent = VERSION + ' (WebGL2)';
}
...
function clampPlayerToBounds() {
  const { width, height } = gameRenderer.getDimensions();
